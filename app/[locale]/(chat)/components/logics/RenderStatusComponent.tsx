  ...
  switch (message?.status) {
    case "seen":
      message?.sender?._id === currentUser?._id && type === "onFriendListCard"
        ? (statusDiv = (
            <div className="h-4 w-4 md:h-4 md:w-4 relative mr-[10px] ">
              <Image
                height={15}
                width={15}
    ...
    case "delivered":
      message.sender?._id !== currentUser?._id && type === "onFriendListCard"
        ? (statusDiv = (
            <div className="h-4 w-4 md:h-4 md:w-4 relative mr-[10px]  rounded-full bg-sky-500 flex items-center justify-center">
