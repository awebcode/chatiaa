...
if (cluster.isPrimary) {
  ...
  io.on("connection", (socket: Socket) => {
    ...
    socket.on("setup", async (userData) => {
      ...
      const worker = new Worker(path.resolve(__dirname, "./worker/online_userworker"), {
        workerData: { userId: userData.userId, socketId: socket.id },
      });

      worker.on("message", (onlineUsersData) => {
