...
import { processChatsWithUnseenCount } from "../common/processChatsWithUnseenCount";
...
export const fetchChats = async (
  ...
  try {
    ...
    const unseenCount = await unseenMessagesCounts(limit, skip, keyword, req.id);
    ...
    if (searchQuery) {
      filteredChats = chats.filter((chat:any) =>
        chat.users.some((user:any) =>
          user.name.match(new RegExp(searchQuery, "i")) ||
          user.email.match(new RegExp(searchQuery, "i"))
        ) || (chat.chatName && chat.chatName.match(new RegExp(searchQuery, "i")))
      );
    }
